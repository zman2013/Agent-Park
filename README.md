...
- **会话中断**：聊天面板顶部停止按钮可立即中断正在运行的 session，自动收尾流式气泡和待发送的 compact 标记
- **聊天附件上传**：输入框支持 `+` 按钮选择本地文件、直接粘贴图片自动上传，附件路径以「附件列表」形式注入 prompt；切换 task 时自动重置 attachments，避免误用上一个 task 的上传
- **自动 Compact**：监控 cco/ccs 的 token 使用，按模型推断 context window（兼容 1M / 200k 模型别名），临近上限时自动注入 compact 指令，阈值以绝对 token 数配置
