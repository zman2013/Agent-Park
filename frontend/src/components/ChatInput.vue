...
3. 执行 \`git fetch\`。
4. 【重要】不要用 \`git stash --include-untracked\` 全量 stash。未跟踪文件里常有巨型产物/备份（几 GB 甚至几十 GB 的 .bin / dump 目录），全量 stash 会极慢并撑爆 .git。大多数未跟踪文件无需处理（rebase 不碰它们），但若目标分支引入的路径与本地未跟踪文件存在**路径冲突**，rebase 会报错。处理方式：用 \`git diff --name-only HEAD...<rebase目标>\` 列出目标分支新增/修改文件，与 step 1 中的 \`??\` 列表做**前缀匹配**（不只是精确匹配）：即目标文件路径是某个本地未跟踪路径的前缀，或本地未跟踪路径是某个目标文件路径的前缀，均为冲突。若有冲突路径，**不要用 \`mv x x.bak\` 就地改名**（可能静默覆盖已存在的同名 \`.bak\`，移回时也可能覆盖 rebase 引入的上游文件或产生文件/目录冲突）；改为用 \`mktemp -d\` 在仓库外创建一个唯一临时目录（记下路径），把每个冲突路径按原始相对路径 \`mkdir -p\` 后移入该临时目录，保持目录结构与来源一一对应。
