...
2. 【重要】不要用 \`git stash --include-untracked\` 全量 stash。未跟踪文件里常有巨型产物/备份（几 GB 甚至几十 GB 的 .bin / dump 目录），全量 stash 会极慢并撑爆 .git。大多数未跟踪文件无需处理（rebase 不碰它们），但若目标分支引入了与本地未跟踪文件**同名**的文件，rebase 会报错 "untracked working tree files would be overwritten"。处理方式：先执行 \`git fetch\`，然后用 \`git diff --name-only HEAD...<rebase目标>\` 列出目标分支新增文件，与 step 1 中的 \`??\` 列表（已逐文件展开）做交集；若有冲突路径，临时移走（\`mv <file> <file>.bak\`）或单独 stash 这些小文件，rebase 完成后再移回。
